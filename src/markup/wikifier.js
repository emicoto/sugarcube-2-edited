...
var Wikifier = (() => { // eslint-disable-line no-unused-vars, no-var
	...
	*******************************************************************************************************************/
	class Wikifier {
		constructor(destination, source, options, passageTitle, passageObj) {
			...
			if (_callDepth === 0) {
				_passageTitleLast = passageTitle;

				if (_passageStack.length !== 0) {
					console.error('Wikifier constructor(): _callDepth === 0, but _passageStack is not empty.', _passageStack);
				}
				_passageStack = [];
				_passageStack.push({ passageTitle : _passageTitleLast });
			}
			else {
				if (passageTitle) {
					_passageTitleLast = passageTitle;
					_passageStack.push({ passageTitle });
				}
				else {
					const lp = _passageStack.last();
			...
			Object.defineProperties(this, {
				passageObj : {
					...
					value    : _passageObj || { title : passageTitle }
				},
			...
			*/
			try {
				...
				const lp = _passageStack.pop();
				_passageTitleLast = lp.passageTitle;

				if (_callDepth === 0) {
					_passageTitleLast = '';
					_passageObj = undefined;

					if (_passageStack.length !== 0) {
						console.error('Wikifier constructor(): finally(_callDepth === 0) _passageStack is not empty.', _passageStack);
					}
					_passageStack = [];
				}
			}
		...
		*/
		static wikifyEval(text, passageTitle, passageObj) {
			const output = document.createDocumentFragment();

			new Wikifier(output, text, undefined, passageTitle, passageObj);
